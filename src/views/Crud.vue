<template>
  <b-container class="crud" fluid="sm">
    <b-row class="justify-content-md-center mb-2">
      <b-col cols="6">
        <b-form class="crud__form  justify-content-md-center" @submit.stop="">
          <b-input-group-prepend aria-label="Toolbar with button groups and input groups">
            ...
            <b-button type="button"  size="md" :variant="isEditMode?'outline-success':'outline-primary'"
              @click.stop="add">
              <b-icon-check v-if="isEditMode" aria-hidden="true"></b-icon-check>
              <b-icon-plus v-else aria-hidden="true"></b-icon-plus>
            </b-button>
